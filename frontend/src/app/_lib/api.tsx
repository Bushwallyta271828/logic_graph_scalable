...
export async function fetchWrapper<T>({path, data}: {path: string, data: T | null}) {
  ...
  noStore(); //Don't store process.env.NEXT_PUBLIC_BACKEND_ADDRESS.
  if (typeof process.env.NEXT_PUBLIC_BACKEND_ADDRESS === 'undefined') {
    throw new Error('NEXT_PUBLIC_BACKEND_ADDRESS undefined');
  } else {
    try {
      ...
      const response = await fetch(process.env.NEXT_PUBLIC_BACKEND_ADDRESS + path, options);
